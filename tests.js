// Test code, meant to be pasted after the fndsa.js contents, so that
// the test code can exercise the private functions of the implementation.
...
function DEBUG(s) {
    ...
    if (++ DEBUG_count > 1000) {
        throw new Error('DEBUG exceeded');
    }
    DEBUG_out.innerHTML += s + '<br />';
}

function hexToBin(s) {
    let bb = new Uint8Array(s.length / 2);
    for (let i = 0; i < bb.length; i ++) {
        bb[i] = parseInt(s.substr(i * 2, 2), 16);
    }
    return bb;
}

function test_SHAKE256_inner(s, d) {
    let sh = new SHAKE(256);
    sh.inject(hexToBin(s));
    sh.flip();
    let e = hexToBin(d);
...
function test_SHAKE256() {
    DEBUG('### Test SHAKE256');
    if (!test_SHAKE256_inner('', '46b9dd2b0ba88d13233b3feb743eeb243fcd52ea62b81b82b50c27646ed5762fd75dc4ddd8c0f200cb05019d67b592f6fc821c49479ab48640292eacb3b7c4be')) {
        return false;
    }
    if (!test_SHAKE256_inner('', '2bac5716803a9cda8f9e84365ab0a681327b5ba34fdedfb1c12e6e807f45284b')) {
        return false;
    }
    if (!test_SHAKE256_inner('8d8001e2c096f1b88e7c9224a086efd4797fbf74a8033a2d422a2b6b8f6747e4', '')) {
        return false;
    }
    return true;
}
...
function test_fpoly() {
    ...
    for (let logn = 1; logn <= 10; logn ++) {
        ...
        let t2 = new Float64Array(n);
        sh.reset();
        sh.inject(hexToBin('a' + logn.toString(16)));
        sh.flip();
        for (let i = 0; i < n; i ++) {
            a[i] = (sh.extract_u16() >>> 5) - 1024;
            b[i] = (sh.extract_u16() >>> 5) - 1024;
        }
        for (let k = 0; k < n; k ++) {
            let s = 0;
            for (let i = 0; i <= k; i ++) {
        ...
        fpoly.FFT(logn, t1);
        fpoly.FFT(logn, t2);
        fpoly.mul_fft(logn, t1, t2);
        fpoly.iFFT(logn, t1);
        for (let i = 0; i < n; i ++) {
            let d = Math.round(t1[i]);
            let e = c[i];
            if (d !== e) {
                DEBUG('BAD: ' + d + ' / ' + e);
                return false;
...
function test_mq() {
    ...
    /*
    for (let x = 1; x <= 12289; x ++) {
        for (let y = 1; y <= 12289; y ++) {
            let z = mq.add(x, y);
            let r = ((x + y + 12288) % 12289) + 1;
            if (z !== r) {
                DEBUG('BAD: add: ' + x + ' + ' + y + ' -> ' + z + ' (exp: ' + r + ')');
                return false;
            }
            z = mq.sub(x, y);
            r = ((x - y + 12288 + 12289) % 12289) + 1;
            if (z !== r) {
                DEBUG('BAD: sub: ' + x + ' + ' + y + ' -> ' + z + ' (exp: ' + r + ')');
                return false;
            }
            z = mq.mmul(x, y);
            r = (((x * y) % 12289) * 11857 + 12288) % 12289 + 1;
            if (z !== r) {
                DEBUG('BAD: mmul: ' + x + ' + ' + y + ' -> ' + z + ' (exp: ' + r + ')');
                return false;
            }
        }

        if (x !== 12289) {
            let t = mq.div(10952, x);
            let u = mq.mmul(t, x);
            if (u !== 1) {
                DEBUG('BAD: inv: ' + x + ' -> ' + t);
                return false;
    ...
    */
    ...
    for (let logn = 2; logn <= 10; logn ++) {
        //DEBUG('logn=' + logn);
        let n = 1 << logn;
        let t1 = new Uint16Array(n);
        let t2 = new Uint16Array(n);
        let t3 = new Uint16Array(n);
        let t4 = new Uint16Array(n);
        let tb = new Int8Array(n);
        let ts = new Int16Array(n);

        let seed = new Uint8Array(3);
        let sh = new SHAKE(256);
        for (let i = 0; i < 16 << (10 - logn); i ++) {
            seed[0] = logn;
            seed[1] = i & 0xFF;
            seed[2] = (i >> 8) & 0xFF;
            sh.reset();
            ...
            sh.flip();
            for (let j = 0; j < n; j ++) {
                let x = sh.extract_u8() - 128;
                if (x === -128) {
                    x = -127;
                }
                tb[j] = x;
            }
            mq.poly_small_to_int(logn, tb, t1);
            for (let j = 0; j < n; j ++) {
                let xf = tb[j];
                let x1 = t1[j];
                let y = (xf <= 0) ? xf + 12289 : xf;
                if (x1 !== y) {
                    DEBUG('BAD small_to_int: ' + xf + ' -> ' + x1);
                    return false;
                }
            }

            for (let j = 0; j < n; j ++) {
                let x = (sh.extract_u16() >>> 4) - 2048;
                if (x === -2048) {
                    x = -2047;
                }
                ts[j] = x;
            ...
            let sqn1 = mq.poly_sqnorm_signed(logn, t4);
            let sqn2 = 0;
            for (let j = 0; j < n; j ++) {
                sqn2 += ts[j] * ts[j];
            }
            if (sqn1 !== sqn2) {
                DEBUG('BAD sqnorm_signed: ' + sqn1 + ' (exp: ' + sqn2 + ')');
                return false;
            }

            t1.set(t4);
            mq.poly_signed_to_int(logn, t1);
            for (let j = 0; j < n; j ++) {
                let xh = ts[j];
                let x1 = t1[j];
                let y = (xh <= 0) ? xh + 12289 : xh;
                if (x1 !== y) {
                    DEBUG('BAD signed_to_int: ' + xh + ' -> ' + x1);
                    return false;
                }
            }

            for (let j = 0; j < n; j ++) {
                t1[j] = sh.extract_u16() % 12289;
                t2[j] = sh.extract_u16() % 12289;
            }

            let sqn3 = mq.poly_sqnorm_ext(logn, t1);
            let sqn4 = 0;
            for (let j = 0; j < n; j ++) {
                let x = t1[j];
                if (x > (12289 >>> 1)) {
            ...
            if (sqn3 !== sqn4) {
                DEBUG('BAD sqnorm_ext: ' + sqn3 + '(exp: ' + sqn4 + ')');
                return false;
            }
            ...
            for (let j = 0; j < n; j ++) {
                let y = (t3[j] === 0) ? 12289 : t3[j];
                if (t1[j] !== y) {
                    DEBUG('BAD ext_to_int: ' + t3[j] + ' -> ' + t1[j]);
                    return false;
                }
            }
            t4.set(t1);
            mq.poly_int_to_ext(logn, t4);
            for (let j = 0; j < n; j ++) {
                if (t4[j] !== t3[j]) {
                    DEBUG('BAD int_to_ext: ' + t1[j] + ' -> ' + t4[j]);
                    return false;
                }
            }

            t4.set(t1);
            mq.poly_int_to_ntt(logn, t4);
            mq.poly_ntt_to_int(logn, t4);
            for (let j = 0; j < n; j ++) {
                if (t4[j] !== t1[j]) {
                    DEBUG('BAD NTT+iNTT: j=' + j + ': ' + t4[j] + ' vs ' + t1[j]);
                    return false;
                }
            }

            t4.set(t2);
            mq.poly_ext_to_int(logn, t2);
            mq.poly_int_to_ntt(logn, t1);
            mq.poly_int_to_ntt(logn, t2);
            mq.poly_mul_ntt(logn, t1, t2);
            mq.poly_ntt_to_int(logn, t1);
            mq.poly_int_to_ext(logn, t1);
            for (let k = 0; k < n; k ++) {
                let s1 = 0;
                for (let j = 0; j <= k; j ++) {
                    s1 += (t3[j] * t4[k - j]) % 12289;
                }
                let s2 = 0;
                for (let j = k + 1; j < n; j ++) {
                    s2 += (t3[j] * t4[k + n - j]) % 12289;
                }
                let s = ((s1 % 12289) + 12289 - (s2 % 12289)) % 12289;
                if (s !== t1[k]) {
                    DEBUG('BAD: ntt+mul: k=' + k + ': ' + t1[k] + ' vs ' + s);
                    return false;
                }
            }
        }
...
function test_sample_f_inner(logn, sref) {
    let n = 1 << logn;
    let sh = new SHAKE(256);
    sh.inject_u8(logn);
    sh.flip();
...
function test_sample_f() {
    DEBUG('### Test sample_f');

    if (!test_sample_f_inner(2,  "e70c150d")) { return false; }
    if (!test_sample_f_inner(3,  "d11108c2093007f9")) { return false; }
    if (!test_sample_f_inner(4,  "071f280109f0f9fca611f6080020f8ef")) { return false; }
    if (!test_sample_f_inner(5,  "19fa150a1008f5011110010d04ec0df50b090c18fd0cfce904f2ea10101120fd")) { return false; }
    if (!test_sample_f_inner(6,  "f8ecf40606fdee0903040a0208ff0b150f04ed040601f808f4f4ecf5f6f8fdedfc06fef9f8f4f1ebf6fdf7f515fb06fffcf808ef03090a0803fb010201011210")) { return false; }
    if (!test_sample_f_inner(7,  "")) { return false; }
    if (!test_sample_f_inner(8,  "")) { return false; }
    if (!test_sample_f_inner(9,  "")) { return false; }
    if (!test_sample_f_inner(10, "")) { return false; }
...
class fake_rng {
    ...
    extract_u8() {
        if (this.ptr >= this.blob.length) {
            throw new Error('Random buffer exhausted');
        }
        return this.blob[this.ptr ++];
    }
    extract_u16() {
    ...
    extract_u32() {
        // Inner test vector uses big-endian for historical reasons.
...
function test_sampler() {
    ...
    let rng = new fake_rng(hexToBin(
        ...
        "E97F27"));
    let mu_invsigma = new Float64Array([
        -9.190471153063714382e+01,
        +5.869236780025413047e-01,
        -8.322564895434936716e+00,
        +5.869236780025413047e-01,
        -1.909651610921680387e+01,
        +5.869983475876336954e-01,
        -1.133554398242332617e+01,
        +5.869983475876336954e-01,
        +7.938673419399755460e+00,
        +5.887669933306912684e-01,
        -2.899085008686725473e+01,
        +5.887669933306912684e-01,
        -9.071257914091654939e+00,
        +5.889010254291510149e-01,
        -4.388754568839566161e+01,
        +5.889010254291510149e-01,
        -5.817435547946094943e+01,
        +5.878554903954469335e-01,
        -4.358664906684732188e+01,
        +5.878554903954469335e-01,
        -3.470565203313314839e+01,
        +5.879106560882698673e-01,
        -4.436009577368896117e+01,
        +5.879106560882698673e-01,
        -2.178303707934623645e+01,
        +5.896780585211260917e-01,
        -3.968827784633828060e+01,
        +5.896780585211260917e-01,
        -1.848860706105684670e+01,
        +5.897875001473220635e-01,
        -4.839610939101591214e+01,
        +5.897875001473220635e-01,
        -1.010038563292495724e+02,
        +5.902510739762089065e-01,
        -5.576620651942215545e+01,
        +5.902510739762089065e-01,
        +4.279820924390406844e+00,
        +5.904219760064700617e-01,
        -6.163233027674051456e+00,
        +5.904219760064700617e-01,
        -1.263690123633507234e+01,
        +7.582363786971733077e-01,
        -3.526593817035737999e+01,
        +7.604192895616016523e-01,
        +7.266419711603845144e+00,
        +7.604192895616016523e-01,
        -5.634534105582309849e+01,
        +7.628615185207271443e-01,
        -4.449414151986549371e+01,
        +7.628615185207271443e-01,
        +1.409718299710782397e+01,
        +7.651495882291911022e-01,
        -6.507066191351185580e+01,
        +7.651495882291911022e-01,
        -1.275178128012327861e+01,
        +7.587794446764489775e-01,
        -2.317843673045300434e+01,
        +7.587794446764489775e-01,
        -4.613300456886798173e+01,
        +7.613063300997616745e-01,
        -1.018699098512950840e+01,
        +7.613063300997616745e-01,
        -2.169345940685706609e+01,
        +7.640588932717317094e-01,
        -5.461834380616269158e+01,
        +7.640588932717317094e-01,
        -3.317287921010942853e+01,
        +7.667484516372335968e-01,
        +1.333215277997257431e+01,
        +7.667484516372335968e-01,
        -3.158003269733924867e+00,
        +7.614590450940674771e-01,
        -4.686944934276761643e+01,
        +7.614590450940674771e-01,
        -1.072677976263620891e+01,
        +7.638455317079316798e-01,
        -3.740892251393602663e+01,
        +7.638455317079316798e-01,
        -3.038723453757679493e+01,
        +7.663127259869872976e-01,
        -7.026372190447548860e+01,
        +7.663127259869872976e-01,
        +9.411399056212799152e+00,
        +7.688694397351595322e-01,
        -5.699209000035098427e+01,
        +7.688694397351595322e-01,
        +2.585573418347714778e+02,
        +5.818713904931462899e-01,
        -6.965468285437063400e+01,
        +5.818713904931462899e-01,
        +8.106804035956896826e+01,
        +5.838127370652338444e-01,
        -1.279122103081975723e+02,
        +5.838127370652338444e-01,
        +5.702070051647674518e+02,
        +5.856984317314140043e-01,
        -3.496875203966805543e+02,
        +5.856984317314140043e-01,
        +6.547701297414964472e+01,
        +5.875340675099811616e-01,
        +3.380993089070585143e+02,
        +5.875340675099811616e-01,
        +1.105683462680004254e+02,
        +5.834809696075538010e-01,
        -2.325180398096597969e+02,
        +5.834809696075538010e-01,
        +6.308131076531856252e+02,
        +5.855348768871343479e-01,
        +2.268111493232144937e+02,
        +5.855348768871343479e-01,
        +5.975144830676753145e+02,
        +5.876519244858485758e-01,
        -2.113004398244640925e+01,
        +5.876519244858485758e-01,
        +4.303493376567207633e+02,
        +5.896089214661741629e-01,
        +4.593383684479874205e+02,
        +5.896089214661741629e-01,
        +1.246394909741142385e+02,
        +5.847002166488463049e-01,
        +3.690955335115287994e+01,
        +5.847002166488463049e-01,
        +1.013706222831420547e+02,
        +5.864539226908561664e-01,
        +4.408177113122501822e+01,
        +5.864539226908561664e-01,
        +1.893416977852795071e+02,
        +5.883374292941855321e-01,
        +1.556954906007113095e+02,
        +5.883374292941855321e-01,
        -1.610124751160346079e+02,
        +5.900312126610068875e-01,
        +4.572753924271371062e+01,
        +5.900312126610068875e-01,
        +4.160125095786951306e+02,
        +5.863936073100981528e-01,
        -2.258990675385898612e+02,
        +5.863936073100981528e-01,
        +6.327954210965491910e+01,
        +5.882331807433952564e-01,
        +2.066076522598786198e+02,
        +5.882331807433952564e-01,
        +2.924962312659678219e+02,
        +5.903531323925853558e-01,
        +9.313083655295582730e+01,
        +6.063886694975748615e-01,
        +6.201026558385988210e+02,
        +6.083565605225011996e-01,
        -2.008389794963595136e+02,
        +6.083565605225011996e-01,
        +2.961329605359904917e+02,
        +6.099240908335331612e-01,
        +2.100479336894326821e+02,
        +6.099240908335331612e-01,
        -2.943037002308304650e+01,
        +6.120371920838471036e-01,
        +4.784612501464640104e+02,
        +6.120371920838471036e-01,
        +1.932034837199007313e+02,
        +6.076303752132461433e-01,
        -2.127876028461080011e+02,
        +6.076303752132461433e-01,
        +2.526275864020178972e+02,
        +6.095899514165163957e-01,
        +1.476250917101831419e+02,
        +6.095899514165163957e-01,
        +4.667966245776611913e+02,
        +6.113952415016870034e-01,
        +1.772290844467436273e+02,
        +6.113952415016870034e-01,
        +4.434087825150309072e+02,
        +6.135067631237066665e-01,
        +7.333486006518996874e+02,
        +6.135067631237066665e-01,
        +2.853157268694864683e+02,
        +6.272757078230520866e-01,
        -1.200220273197629268e+02,
        +6.272757078230520866e-01,
        +1.563282245641328814e+02,
        +6.314082692006764974e-01,
        +7.772762801761474272e+00,
        +6.314082692006764974e-01,
        +2.816676383062125524e+02,
        +6.298640274246237736e-01,
        +1.332640726304600207e+02,
        +6.298640274246237736e-01,
        -4.809735098728561411e+02,
        +6.336311318283954774e-01,
        +3.071844282837032836e+01,
        +6.336311318283954774e-01,
        +3.809706892450373061e+02,
        +6.289048700288534244e-01,
        -1.057019905377560747e+02,
        +6.289048700288534244e-01,
        +2.000461110824175819e+02,
        +6.330956242959540070e-01,
        -9.210236401010284624e+01,
        +6.330956242959540070e-01,
        +1.295515990686150758e+02,
        +6.317946370759104369e-01,
        +7.039312588757296396e+01,
        +6.317946370759104369e-01,
        +2.454968660301977934e+02,
        +6.355965895894984952e-01,
        +1.086896251215169116e+02,
        +6.355965895894984952e-01,
        +4.551798914643523375e+01,
        +6.312697781021546772e-01,
        -8.798728515543538720e+01,
        +6.312697781021546772e-01,
        +2.387901372252069052e+02,
        +6.350109774691831133e-01,
        +1.279689615583139073e+02,
        +6.350109774691831133e-01,
        +6.166352663681948343e+02,
        +6.335464793372626024e-01,
        +9.869851585602447130e+01,
        +6.335464793372626024e-01,
        +1.600886339267818528e+02,
        +6.370334234073591340e-01,
        +2.651038254811689399e+02,
        +6.370334234073591340e-01,
        +6.545844019682117505e+02,
        +6.326989899849853050e-01,
        +1.254874044758294644e+02,
        +6.326989899849853050e-01,
        +1.726493949905457441e+02,
        +6.364675021813192002e-01,
        -2.445111655640675394e+02,
        +6.364675021813192002e-01,
        +4.293512677751457431e+02,
        +6.352307263592679165e-01,
        +1.600996472287626204e+02,
        +6.352307263592679165e-01,
        +2.649074254095724612e+01,
        +6.387287039583695591e-01,
        +1.577404664447418270e+02,
        +6.387287039583695591e-01,
        +1.984536473450530991e+02,
        +6.266892073358877324e-01,
        -1.609042817276809103e+02,
        +6.266892073358877324e-01,
        +2.766714395358624756e+02,
        +6.277367115006877718e-01,
        -1.594610164778390526e+02,
        +6.277367115006877718e-01,
        +5.034432013963373151e+02,
        +6.305914663468268344e-01,
        +9.306270254702320699e+01,
        +6.305914663468268344e-01,
        +2.820855365856236290e+02,
        +6.317371944021727970e-01,
        +1.601434972844399454e+02,
        +6.317371944021727970e-01,
        +4.968236887304136644e+02,
        +6.287668330102123004e-01,
        +3.025565344769993317e+02,
        +6.287668330102123004e-01,
        +2.107601125734344407e+02,
        +6.299710485789209446e-01,
        +1.924656264150157483e+02,
        +6.299710485789209446e-01,
        +1.411884564326072962e+02,
        +6.331067325999172324e-01,
        +4.191150637791374720e+02,
        +6.331067325999172324e-01,
        +4.101037559676462934e+02,
        +6.344517965159000017e-01,
        +4.869372768725125979e+02,
        +6.344517965159000017e-01,
        +2.427813277663778422e+02,
        +6.305248608386845310e-01,
        -1.126381439478294055e+02,
        +6.305248608386845310e-01,
        +6.864473614828953885e+01,
        +6.314951549671563580e-01,
        -2.669323137195392519e+02,
        +6.314951549671563580e-01,
        +6.441495342777618589e+02,
        +6.339785631307883751e-01,
        +4.656181837860988253e+02,
        +6.339785631307883751e-01,
        +1.886211376528917754e+02,
        +6.350460628276881625e-01,
        +4.417624090790843638e+02,
        +6.350460628276881625e-01,
        +1.569495558474984875e+02,
        +6.324370305083251154e-01,
        -7.454314960571809934e+01,
        +6.324370305083251154e-01,
        +1.184439149903806481e+02,
        +6.335407549449221243e-01,
        -9.227533305537933472e+01,
        +6.335407549449221243e-01,
        +1.218023830798407801e+02,
        +6.363201845724248962e-01,
        -1.346623030045439862e+02,
        +6.363201845724248962e-01,
        +3.436058180430233620e+02,
        +6.375616765818532539e-01,
        +1.427247627133343144e+02,
        +6.375616765818532539e-01,
        -5.527353758748702006e+01,
        +6.490336329216853661e-01,
        -9.140607902026837905e+01,
        +6.490336329216853661e-01,
        +3.510727190421183650e+02,
        +6.512545050974540839e-01,
        +9.518217073452923671e+01,
        +6.512545050974540839e-01,
        +4.133592751121906872e+02,
        +6.506788414997292103e-01,
        -1.795186098084115542e+02,
        +6.506788414997292103e-01,
        +1.650687612792767993e+02,
        +6.527638215066879912e-01,
        +3.564194530246986687e+02,
        +6.527638215066879912e-01,
        -1.346987244059463933e+02,
        +6.511493559007520693e-01,
        -1.962779099354561367e+02,
        +6.511493559007520693e-01,
        +5.748573352937490881e+02,
        +6.535333431584177122e-01,
        +4.591660860440019860e+02,
        +6.535333431584177122e-01,
        +4.414470279598230036e+02,
        +6.530134716471553968e-01,
        -8.334061196624512036e+01,
        +6.530134716471553968e-01,
        -1.595192842147002068e+02,
        +6.552685793215194954e-01,
        +2.196568356364373358e+02,
        +6.552685793215194954e-01,
        -5.801676699606335319e+01,
        +6.514638941442399966e-01,
        -1.303838055344546376e+02,
        +6.514638941442399966e-01,
        +5.676533057325957543e+02,
        +6.534285387168462522e-01,
        +3.473096861660250738e+02,
        +6.534285387168462522e-01,
        +4.885553214998506064e+02,
        +6.529635042339219098e-01,
        -7.493048019720598063e+01,
        +6.529635042339219098e-01,
        +2.185808412322442393e+02,
        +6.548212284681204087e-01,
        +3.469507922504965336e+02,
        +6.548212284681204087e-01,
        +7.135631542539471184e+01,
        +6.533969208888383839e-01,
        -2.615757141879323058e+02,
        +6.533969208888383839e-01,
        +4.328500862922156216e+02,
        +6.726597255701335376e-01,
        +3.151966593337035647e+02,
        +6.726597255701335376e-01,
        +3.825348470957458176e+02,
        +6.681351391142733576e-01,
        +2.249415873309847314e+02,
        +6.681351391142733576e-01,
        +1.790950891357846899e+02,
        +6.706466045937254927e-01,
        +3.152760283582697980e+02,
        +6.706466045937254927e-01,
        +2.267727348782384240e+02,
        +6.701361412532842454e-01,
        +1.440233901920956612e+02,
        +6.701361412532842454e-01,
        +7.090951310917678541e+01,
        +6.725805792530333838e-01,
        +1.776179028652021543e+02,
        +6.725805792530333838e-01,
        -4.143489062717458893e+01,
        +6.697594298334138552e-01,
        -7.973439558235631353e+01,
        +6.697594298334138552e-01,
        -2.191515090047795411e+02,
        +6.723576906483268445e-01,
        +3.692478171547756460e+01,
        +6.723576906483268445e-01,
        -2.895283530234202729e+01,
        +6.719933451902534438e-01,
        -1.419850020632923204e+01,
        +6.719933451902534438e-01,
        +3.342011809649431484e+02,
        +6.745291056691186116e-01,
        +2.817973864640247825e+02,
        +6.745291056691186116e-01,
        +4.617605155562361006e+02,
        +6.897462854001495947e-01,
        -1.478766060978230144e+02,
        +6.897462854001495947e-01,
        +1.612251309934227947e+02,
        +6.897696056858044766e-01,
        -1.484649337281116175e+02,
        +6.897696056858044766e-01,
        +3.207369311039425952e+02,
        +6.922266264700329685e-01,
        +3.493615278560019988e+01,
        +6.922266264700329685e-01,
        +2.510289539410107977e+02,
        +6.922277052901685268e-01,
        +1.041658435625129897e+02,
        +6.922277052901685268e-01,
        +2.020251942562580041e+02,
        +6.909506670184404342e-01,
        +2.417595445800617142e+02,
        +6.909506670184404342e-01,
        +4.975560444912666753e+02,
        +6.909700021666579373e-01,
        -1.557345976333768078e+02,
        +6.909700021666579373e-01,
        +4.305598355564953863e+02,
        +6.933503467023287570e-01,
        -2.301788039851253131e+02,
        +6.933503467023287570e-01,
        +1.279436673807245768e+02,
        +6.933509666694475104e-01,
        +2.341723962945972062e+02,
        +6.933509666694475104e-01,
        +2.242431599727306946e+02,
        +6.960293110930423355e-01,
        +6.001501664736478006e+02,
        +6.960293110930423355e-01,
        +8.193192867453829820e+01,
        +6.960341638281182242e-01,
        -7.988447396610789042e+01,
        +6.960341638281182242e-01,
        +3.771612306578181801e+02,
        +6.983273351300477438e-01,
        +1.301821939137403206e+02,
        +6.983273351300477438e-01,
        +3.411544261979150860e+02,
        +6.983285266591753304e-01,
        +1.619869174381567234e+02,
        +6.983285266591753304e-01,
        +2.287795436747664723e+02,
        +6.970292659770271904e-01,
        +6.431430783727458333e+01,
        +6.970292659770271904e-01,
        +3.288779396037517131e+02,
        +6.970331705807737421e-01,
        -5.284890612484089445e+01,
        +6.970331705807737421e-01,
        +2.478324743878071672e+02,
        +6.992633209009220252e-01,
        -1.081471413014033089e+01,
        +6.992633209009220252e-01,
        +4.690807881813481686e+02,
        +6.992646317605394346e-01,
        +3.710903231608759825e+02,
        +6.992646317605394346e-01,
        +2.086674824993144028e+01,
        +7.085215048996459375e-01,
        +6.999826089031341780e+01,
        +7.085215048996459375e-01,
        +7.586904812575060442e-01,
        +1.404807638406917079e+02,
        +7.586904812575060442e-01,
        +3.929004394842842203e+02,
        +7.609712893845126391e-01,
        +2.859575522346099206e+02,
        +7.609712893845126391e-01,
        +3.153580670424570940e+02,
        +7.610427006566415908e-01,
        +4.975653062194700738e+02,
        +7.610427006566415908e-01,
        +3.391752062156745069e+02,
        +7.596915452479685582e-01,
        -7.025263508343707031e+01,
        +7.596915452479685582e-01,
        +2.112358454646619634e+02,
        +7.598644881152574193e-01,
        -3.565415069095018907e+02,
        +7.598644881152574193e-01,
        -3.511146107231396485e+01,
        +7.621539853476249071e-01,
        +1.200891845024361828e+02,
        +7.621539853476249071e-01,
        +6.687919799498737916e+01,
        +7.622509480771845203e-01,
        +6.287067669172932938e+02,
        +7.622509480771845203e-01 ]);
    let refout = new Int16Array([
    ...
    let ss = new sampler(9, rng);
    ss.sample_be = true;  // historical test vector uses big-endian
    let num = mu_invsigma.length >> 1;
    for (let i = 0; i < num; i ++) {
        let mu = mu_invsigma[(i << 1) + 0];
        let isigma = mu_invsigma[(i << 1) + 1];
        let v = ss.next(mu, isigma);
        let w = refout[i];
        if (v != w) {
            DEBUG('BAD: i = ' + i + ' -> ' + v + ' (exp: ' + w + ')');
            return false;
        }
    }
    if (rng.ptr != rng.blob.length) {
        DEBUG('BAD: not all randomness was consumed');
        return false;
    }
    return true;
}
...
function test_sign() {
    ...
    for (let logn = 2; logn <= 10; logn ++) {
        //DEBUG('logn=' + logn);
        let kp = core.keygen(logn);
        let sk = kp.sign_key;
        let vk = kp.verify_key;
        for (let i = 0; i < 10; i ++) {
            let msg = new TextEncoder().encode('test' + i);
            let sig = core.sign(sk, '', core.ID_RAW, msg);
            if (sig.length != core.signature_length(logn)) {
                DEBUG('BAD: signature length = ' + sig.length);
                return false;
            }
            if (!core.verify(sig, vk, '', core.ID_RAW, msg)) {
                DEBUG('BAD: failed to verify signature');
                return false;
            }
        }
    }
...
function test_verify_inner(hex_vk, hex_msg, hex_sig) {
    let vk = hexToBin(hex_vk);
    let msg = hexToBin(hex_msg);
    let sig = hexToBin(hex_sig);

    if (!core.verify(sig, vk, '', core.ID_ORIG, msg)) {
        DEBUG('fail verify 1');
        return false;
    }
    msg[0] ^= 0x01;
    ...
    msg[0] ^= 0x01;
    if (sig.length <= core.signature_length(8)) {
        for (let i = 0; i < (sig.length << 3); i ++) {
            sig[i >>> 3] ^= 1 << (i & 7);
            if (core.verify(sig, vk, '', core.ID_ORIG, msg)) {
                DEBUG('fail verify 3, i = ' + i);
                return false;
            }
            sig[i >>> 3] ^= 1 << (i & 7);
        }
...
function test_verify() {
    ...
    if (!test_verify_inner(
        "05508e10b20b8dd482f68ce392d9560208cc4a47e25e4289b6c62adea829b5fe087b9a514c850f950225303b81dc874dae61af7a4649518e05",
        "626c6168",
        "35493f3cbff63fb67a454abf115661388d7769fb80807a1a66da8158eeb53eb3f500806bfbba49d47d02d5c15203434e7ee9e8afd9be922a1fe78e8fcb583c92e5e2cbe8b679dfccfb989f4013eab4000000")) {
        return false;
    }
...
export function run_tests(out_obj) {
